import { q as getLayerIdFromUrl, r as isNil, t as isFeatureService, g as getProp, c as cloneObject, u as includes, v as isMapOrFeatureServerUrl, w as getItemHomeUrl, x as getItemApiUrl, y as getItemDataUrl, z as getItemThumbnailUrl, A as createOperationPipeline, O as OperationStack, B as getFamily, C as normalizeItemType, D as isSlug, E as addContextToSlug, F as hubApiRequest, G as parseDatasetId, H as mergeObjects, d as datasetToContent, i as itemToContent, I as getItemHubId } from './index-5ebfac40.js';
import { q as getItemMetadata, t as getItemGroups, v as getUser, h as getItemData, f as getItem } from './index-bed14788.js';
import { g as getService, a as getAllLayersAndTables, p as parseServiceUrl } from './index-d6e159fc.js';
...
var getTraversalObj$1 =function (xmlData,options){
...
var xml2json = function (xmlData,options){
    options = Object.assign({},defaultOptions$2,options);
    return convertToJson$1(getTraversalObj$1(xmlData,options), options);
};
...
var convertToJson$1 = function (node, options){
    var jObj = {};

    //traver through all the children
    for (var index = 0; index < node.child.length; index++) {
        var prop = node.child[index].tagname;
        var obj = convertToJson$1(node.child[index],options);
...
var parse$1 = xml2json;
var getTraversalObj_1 = getTraversalObj$1;
var convertToJson_1 = convertToJson$1;
...
var buildOptions = function (options){
    ...
    for (var i = 0; i < props.length; i++) {
        if(options[props[i]] === undefined){
            options[props[i]] = defaultOptions$1[props[i]];
        }
    }
    return options;
};
...
var parse = x2j.parse;
var getTraversalObj = x2j.getTraversalObj;
var convertToJson = x2j.convertToJson;
 ...
 */
function getContentMetadata(id, requestOptions) {
    return getItemMetadata(id, requestOptions).then(metadataXml => parseMetadataXml(metadataXml));
}

const getLayer = (content, layerId) => {
...
const shouldUseLayerInfo = (content) => {
...
function getMetadataPath(identifier) {
    ...
    const metadataPaths = {
        updateFrequency: "metadata.metadata.dataIdInfo.resMaint.maintFreq.MaintFreqCd.@_value",
        ...
        metadataUpdateFrequency: "metadata.metadata.mdMaint.maintFreq.MaintFreqCd.@_value",
        metadataUpdatedDate: "metadata.metadata.mdDateSt",
    };
    return metadataPaths[identifier];
}
function getValueFromMetadata(content, identifier) {
...
function _enrichDates(content) {
    ...
    // updateFrequency:
    const updateFrequencyValue = getValueFromMetadata(newContent, "updateFrequency");
    if (updateFrequencyValue) {
        newContent.updateFrequency = updateFrequencyMap[updateFrequencyValue];
    }
    // metadataUpdateFrequency
    const metadataUpdateFrequencyValue = getValueFromMetadata(newContent, "metadataUpdateFrequency");
    ...
    const metadataUpdatedDate = parseISODateString(getValueFromMetadata(newContent, "metadataUpdatedDate"));
    if (metadataUpdatedDate) {
        ...
        newContent.metadataUpdatedDateSource = getMetadataPath("metadataUpdatedDate");
    }
    else {
    ...
    const reviseDate = parseISODateString(getValueFromMetadata(newContent, "reviseDate"));
    const layerLastEditDate = getProp(newContent, "layer.editingInfo.lastEditDate");
    const serverLastEditDate = getProp(newContent, "server.editingInfo.lastEditDate");
    ...
    const pubDate = parseISODateString(getValueFromMetadata(newContent, "pubDate"));
    const createDate = parseISODateString(getValueFromMetadata(newContent, "createDate"));
...
const enrichGroupIds = (input) => {
    ...
    return getItemGroups(content.id, requestOptions)
        ...
        stack.finish(opId);
        return {
            data: Object.assign(Object.assign({}, content), { groupIds }),
            stack,
            requestOptions,
        };
    })
...
const enrichMetadata = (input) => {
    ...
    return getContentMetadata(content.id, requestOptions)
        .then((metadata) => {
        stack.finish(opId);
        const data = Object.assign(Object.assign({}, content), { metadata });
        return { data, stack, requestOptions };
    })
        .catch((error) => handleEnrichmentError(error, input, opId));
};
const enrichOwner = (input) => {
    const { data: content, stack, requestOptions } = input;
    ...
    const options = Object.assign({ username: content.owner }, requestOptions);
    return getUser(options)
        .then((ownerUser) => {
        stack.finish(opId);
        const orgId = ownerUser.orgId;
        // TODO: also set owner user
        const data = Object.assign(Object.assign({}, content), { orgId });
        return { data, stack, requestOptions };
    })
        .catch((error) => handleEnrichmentError(error, input, opId));
};
const enrichData = (input) => {
    ...
    return getItemData(content.id, requestOptions)
        .then((itemData) => {
        stack.finish(opId);
        const data = Object.assign(Object.assign({}, content), { data: itemData });
        return { data, stack, requestOptions };
    })
        .catch((error) => handleEnrichmentError(error, input, opId));
};
const enrichServer = (input) => {
    ...
    return getService(options)
        .then((server) => {
        stack.finish(opId);
        const data = Object.assign(Object.assign({}, content), { server });
        return { data, stack, requestOptions };
    })
        .catch((error) => handleEnrichmentError(error, input, opId));
};
const enrichLayers = (input) => {
    ...
    return (getAllLayersAndTables(options)
        ...
        .then((layers) => {
        stack.finish(opId);
        const data = Object.assign(Object.assign({}, content), { layers });
        return { data, stack, requestOptions };
    })
        .catch((error) => handleEnrichmentError(error, input, opId)));
};
...
const handleEnrichmentError = (error, input, opId) => {
    ...
    const data = Object.assign(Object.assign({}, content), { errors });
    return { data, stack, requestOptions };
};
...
const isHubCreatedContent = (content) => {
    ...
    return (content.type === "Web Map" &&
        contentTypeKeywords.some((typeKeyword) => hubTypeKeywords.indexOf(typeKeyword) > -1));
};
const shouldFetchOrgId = (content) => {
...
const getMissingEnrichments = (content) => {
    const enrichments = [];
    if (!content.groupIds) {
 ...
 * @export
 */
const getPortalUrls = (content, requestOptions) => {
 ...
 */
const fetchEnrichments = (content, requestOptions) => {
    // get the list of enrichments (property names) to fetch
    const enrichments = (requestOptions && requestOptions.enrichments) ||
 ...
 */
const enrichContent = (content, requestOptions) => {
    // ensure defaults and enrich content with portal urls
    const defaults = {
        errors: [],
    };
    const contentWithPortalUrls = Object.assign(Object.assign(Object.assign({}, defaults), content), getPortalUrls(content, requestOptions));
    // fetch any missing or requested enrichments
    return fetchEnrichments(contentWithPortalUrls, requestOptions).then((fetched) => {
 ...
 */
const getLayerContent = (content, layerId) => {
    ...
    const normalizedType = normalizeItemType(Object.assign(Object.assign({}, item), { type }));
    const layerContent = Object.assign(Object.assign({}, content), { layer, type, family, normalizedType });
 ...
 * @export
 */
function getContentFromHub(identifier, options) {
    let request;
    if (isSlug(identifier)) {
        const slug = addContextToSlug(identifier, options && options.siteOrgKey);
        const opts = cloneObject(options);
        opts.params = Object.assign(Object.assign({}, opts.params), { "filter[slug]": slug });
        request = hubApiRequest(`/datasets`, opts).then((resp) => resp && resp.data[0]);
    }
    else {
        request = hubApiRequest(`/datasets/${identifier}`, options).then((resp) => resp && resp.data);
    }
    return request
        .then((dataset) => {
        // only if authed
        if (dataset && options.authentication) {
            ...
            return getItem(parseDatasetId(dataset.id).itemId, options).then((item) => {
                dataset.attributes = mergeObjects(item, dataset.attributes, itemOverrides);
                return dataset;
            });
        }
        else {
...
function withPortalUrls(content, requestOptions) {
    /* tslint:disable no-console */
    console.warn("DEPRECATED: Use getPortalUrls() instead. withPortalUrls will be removed at v9.0.0");
    const portalUrls = getPortalUrls(content, requestOptions);
    return Object.assign(Object.assign({}, content), portalUrls);
}
/**
 ...
 * @param options - request options that may include authentication
 */
function getContentFromPortal(idOrItem, requestOptions) {
    let getItemPromise;
    ...
    if (typeof idOrItem === "string") {
        ...
        if (layerIdString) {
            ...
            enrichContentOptions = Object.assign({ layerId }, requestOptions);
        }
    }
    else {
 ...
 * @param options - request options that may include authentication
 */
function getContentById(identifier, options) {
    let getContentPromise;
 ...
 * @param options - request options that may include authentication
 */
function getContent(idOrModel, options) {
    let getContentPromise;
