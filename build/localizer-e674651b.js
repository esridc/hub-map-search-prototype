...
const _typeof$2 = /*@__PURE__*/getDefaultExportFromCjs(_typeof_1);
...
if (typeof commonjsRequire !== 'undefined' && (typeof window === 'undefined' || typeof window.document === 'undefined')) {
  var f = fetchApi || browserPonyfill;
...
const fetchNode = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), getFetch, {
  'default': getFetch
}));
...
const translationResourcesPath = getMetaContent('hub-components-assets-url') || '/assets/i18n';
// Currently supported languages
const supportedLanguages = ['ar', 'bs', 'ca', 'cs', 'da', 'de', 'el', 'en', 'es', 'et', 'fi',
  'fr', 'he', 'hr', 'hu', 'id', 'it', 'ja', 'ko', 'lt', 'lv', 'nb', 'nl', 'pl', 'pt-br', 'pt-pt',
  'ro', 'ru', 'sl', 'sr', 'sv', 'th', 'tr', 'uk', 'vi', 'zh-cn', 'zh-hk', 'zh-tw'
];
// This is consistent with locale sevice of opendata-ui
const rtlLanguages = ['ar', 'he'];
i18next.use(Backend).init({
  supportedLngs: supportedLanguages,
  fallbackLng: 'en',
  backend: {
    loadPath: `${translationResourcesPath}/{{ns}}.i18n.{{lng}}.json`
  },
  load: 'all',
  lowerCaseLng: true,
  ns: []
});
function componentLanguage(element) {
  const closestElement = element.closest('[lang]');
  return closestElement ? closestElement.lang.toLowerCase() : getRootLanguage();
}
function rtl(lang) {
...
function getLocale(language) {
  ...
  const langComponent = browserLocale ? browserLocale.substring(0, 2) : null;
  // If browser locale cannot be obtained, or if it conflicts with provided language, default language if supported
  if (!langComponent || !langComponent.includes(language)) {
    return supportedLanguages.includes(language) ? language : null;
  }
  // Otherwise default to locale, if supported
  return supportedLanguages.includes(langComponent) ? browserLocale : null;
}
function getMetaContent(name) {
  const metaCollection = document.getElementsByTagName('meta');
  for (let i = 0; i < metaCollection.length; i++) {
    if (metaCollection[i].getAttribute('name') === name) {
      return metaCollection[i].getAttribute('content');
    }
  }
}
function getRootLanguage() {
  const root = document.getElementsByTagName('html');
  if (root.length > 0) {
    const rootLang = root[0].getAttribute('lang');
    return rootLang ? rootLang.toLowerCase() : 'en';
  }
  return 'en';
}
// Lifted from opendata-ui implementation
function getBrowserLocale() {
  const nav = window.navigator;
  // support for HTML 5.1 "navigator.languages"
  const navLanguage = _getNavigatorSupportedLanguage(nav);
  if (navLanguage) {
    return navLanguage;
  }
  // support for other well known properties in browsers
  const languageFromBrowserProp = _getLocaleFromBrowserProps(nav);
  if (languageFromBrowserProp) {
    return languageFromBrowserProp;
  }
  return null;
}
function _getNavigatorSupportedLanguage(nav) {
...
function _getLocaleFromBrowserProps(nav) {
  let language;
...
export { componentLanguage as c, getLocale as g, i18next as i };
